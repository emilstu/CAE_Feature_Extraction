...
if __name__ == '__main__':
    ...
    cae_parser.add_argument('-st','--sampler_type', required=True, choices=['grid', 'label'],
                            help='Sampler type')
    cae_parser.add_argument('-mlb','--min_lab_vox', required=False, default=1.0, type=apu.check_range, 
    ...
    cae_parser.add_argument('-rs','--resample', required=False, default=(), type=str,
    ...
    cae_parser.add_argument('-pp','--prepare_batches', required=False,  default=False, dest='prepare_batches', action='store_true',
    ...
    ase_parser.add_argument('-ps', '--patch_size', required=True, type=str,
                            help='Patch size used when the model was trained: "(int,int,int)"')
    ase_parser.add_argument('-po', '--patch_overlap', required=True, type=str, 
    ...
    fe_parser.add_argument('-po', '--patch_overlap', required=False, type=str, 
                            help='Patch overlap 2D/3D: "(0,int,int)" or "(int,int,int)". Must be even number and smaller than patch size')
    ...
    fe_parser.add_argument('-rs','--resample', required=False, default=(), type=str,
                             help='Resample to common voxel spacing (float,float,float)')
    fe_parser.add_argument('-elm','--encoded_layer_num', required=True, type=int,
    ...
    svm_parser.add_argument('-dd', '--data_dir', required=False, default='data/classification/patients/', type=str,
                            help='Directory where data is stored')
    svm_parser.add_argument('-fd', '--feature_dir', required=True, type=str,
                            help='Directory where features are stored, i.e. output from FeEx')
    svm_parser.add_argument('-ffd','--ffr_dir', required=False, default='data/classification/ffr_data/', type=str,
                            help='Directory ffr_values are stores')
    svm_parser.add_argument('-ffn','--ffr_filename', required=True, type=str,
                            help='Filename for the file where ffr-values are stored')
    svm_parser.add_argument('-ffco','--ffr_cut_off', required=False, default=0.85, type=str,
                            help='Filename for the file where ffr-values are stored')
    svm_parser.add_argument('-ts','--test_size', required=False, default=0.4, type=apu.check_range, 
